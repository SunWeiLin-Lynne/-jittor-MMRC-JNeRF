...
def voxelize_sub1(faces, voxels):
    return jt.code([faces], [voxels], 
    cuda_header='''
#include <cuda.h>
...
const auto voxel_size = voxels_shape1;
const int threads = 512;
const dim3 blocks ((batch_size * voxel_size * voxel_size - 1) / threads +1);

voxelize_sub1_kernel<float32><<<blocks, threads>>>(
    faces_p,
    voxels_p,
    batch_size,
    num_faces,
...
def voxelize_sub2(faces, voxels):
    return jt.code([faces], [voxels],
    cuda_header='''
#include <cuda.h>
#include <cuda_runtime.h>
...
voxelize_sub2_kernel<float32><<<blocks, threads>>>(
    faces_p,
    voxels_p,
    batch_size,
    num_faces,
...
def voxelize_sub3(faces, voxels, visible):
    return jt.code([faces], [voxels, visible],
    cuda_header='''
#include <cuda.h>
#include <cuda_runtime.h>
...
@alias(voxels, out0)
@alias(visible, out1)
const auto batch_size = voxels_shape0;
const auto voxel_size = voxels_shape1;
const int threads = 512;
const dim3 blocks ((batch_size * voxel_size  * voxel_size  * voxel_size - 1) / threads +1);

voxelize_sub3_kernel<float32><<<blocks, threads>>>(
    voxels_p,
    visible_p,
    batch_size,
    voxel_size);
...
def voxelize_sub4(faces, voxels, visible):
    return jt.code([faces], [voxels, visible],
    cuda_header='''
#include <cuda.h>
#include <cuda_runtime.h>
...
voxelize_sub4_kernel<float32><<<blocks, threads>>>(
    voxels_p,
    visible_p,
    batch_size,
    voxel_size);
